...
function jsShowReiting(manager)
{
 ...
 $('.accordion2').html('');
 lnk = "do.php?ShowReiting="+manager;
 alert(lnk);
 page = 0;
	    ...
	    var ls=jQuery("#le_table").jqGrid({
            ...
            colModel :[
                ...
                ,{name:'job', index:'job', width:190, editable:true, edittype:"select",editoptions:
                     {value:"Генеральный директор:Генеральный директор;Директор:Директор;Руководитель ОП:Руководитель ОП;Старший менеджер:Старший менеджер;Менеджер:Менеджер;Кредитный эксперт:Кредитный эксперт;Логист:Логист;Бухгалтер:Бухгалтер;Маркетолог:Маркетолог;Администратор:Администратор;Другое:Другое;Уволен:Уволен"}}
...
if (rt.children('.roundfooter2[id="tst"]').html() != data.amount[4] && (rt.children('.roundfooter2[id="tst"]').html()!='')) { rt.children('.roundfooter2[id="tst"]').css("background","#712115"); n=1;}		
  ...
  if (refreshPlot!=0) //Если есть обновления, обновляем график и показываем сообщение
	    ...
	    $(".roundfooter2").each(function()
		  {
		  if ($(this).css('background')=='rgb(218, 87, 0)') 
		     {
		     if ($(this).attr('id')=='dg') 
		       { texttocup = 'Договора: '+$(this).html();
		       jsAlert('simple',0,$(this).parent('h5').attr('id'),$(this).parent('h5').attr('brandtitle')+':',texttocup);
...
//////////////////////////////////////////////////////////////////////////
    ...
    //Подготовка чата
	jsChatRefresh();
    jsSetInterval(3000); //Запуск чата
