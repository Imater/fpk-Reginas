...
//			alert(manager+' '+id);
			$txt2 = $.ajax({type: "GET",url: "do.php?AddAdmin="+id+"&manager="+encodeURIComponent(manager),success:function(){
...
$('.pane-stat[id='+h3_brand+'] input[name=group1]').click(function()
  {
  	   $('.pane-stat[id='+h3_brand+'] input[name=group1]').attr('checked','');
...
month = "0" + month;
  ...
  clearInterval(si2);
  si2=setInterval(jsSmsCup,5*60*1000);		
...
setTimeout(function() { 
						$(".paneto[id="+$txt.responseText+"] .client_tab:first").show();
$(".paneto[id="+$txt.responseText+"] .clientform").show(); 
$(".paneto[id="+$txt.responseText+"] .clientformmini").hide();  
$(".paneto[id="+$txt.responseText+"] input[id=FIO]").focus();
...
function jsCreateChat(iduser,nameuser, height1)
{
	  ...
	  $("* #chat[who='"+iduser+"']").height(300).show().animate({"height": height1}, 400);
								...
								if (height==null) 
								   {
									...
									jsCreateChat(data.m.user,data.m.name,300);
									//alert('Новое сообщение в чате.');
								   }
	   								var chat_user = $("* #chat[who='"+data.m.user+"']");

								   chat_user.find("#chattop").css('background','#009900');
								   chat_user.find("#chattext").append(div_data).scrollTop(100000);
