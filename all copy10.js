 ...
 setTimeout(function()
  {
			...
			$('#notreaded h3:first').css("-webkit-border-top-left-radius","10px 10px").css("-webkit-border-top-right-radius","10px 10px").css("border-top","none");
...
function jshCars( field )
{
 ...
 if (radio!='do')
	tr = setTimeout(function(){
			...
			$('h3:first').css("-webkit-border-top-left-radius","10px 10px").css("-webkit-border-top-right-radius","10px 10px").css("border-top","none");
...
{
         $('.accordion2').html('<div style="margin-top:15px;margin-right:15px;margin-bottom:30px; height:80%;width:100%;font-size:120%;"><table id="le_table"></table><div id="le_tablePager" style="font-size:80%"></div><font color="white">* Сокр.Модель - короткое название модели не более 7 букв (пример: "TepeeVP")<br>* 1-показывать - означает, отображать в "падающем списке" при выборе модели<br>* Вы не cможете удалить модель, если она используется в клиентах</font></div>');
        var lastSel;
	    var ls=jQuery("#le_table").jqGrid({
            url:'getmodel.php',
            datatype: 'json',
            mtype: 'POST',
            colNames:['id', 'Модель', 'Сокр.Модель', 'Средняя цена', '(1-показывать,0-скрыть)', 'Клиентов на эту модель'],
            colModel :[
	...
	setTimeout(function()
		{
		 admin = encodeURIComponent($('#selectmanager').html());
           ...
           $(".accordion2 h5").unbind('click').click(function()
			...
			$('h5:first').css("-webkit-border-top-left-radius","10px 10px").css("-webkit-border-top-right-radius","10px 10px").css("border-top","none");
...
function jsShowStatManager()
{
	    ...
	    var ls=jQuery("#le_table").jqGrid({
            ...
            datatype: 'json',
            mtype: 'POST',
            colNames:['id', 'Менеджер', 'Выдачи', 'Договора', 'Тесты', 'Визиты', 'Звонки', 'Менеджер', 'Out', 'Расторжения', 'Длина выдачи', '% кредитов', 'Ср.цена'],
            colModel :[
                {name:'id', index:'id', width:30}
...
{
 alldate = $('#datediv').html();
 lnk = "do.php?newscup=888&date="+alldate+"&jsSmsCup";
   $.getJSON( lnk ,function(data)
	{
	var texttocup='';
    var refreshPlot=0;
...
if (rt.children('.roundfooter2[id="tst"]').html() != data.amount[4] && (rt.children('.roundfooter2[id="tst"]').html()!='')) { rt.children('.roundfooter2[id="tst"]').css("background","#712115"); n=1;}		
		...
		var plan_month = data.amount[8]?data.amount[8]:"?";
		
		rt.children(".plan_of_month").html("план: "+plan_month);
...
//	jsShowCars();
	...
	$('*').undelegate(".plan_of_month", "click").delegate(".plan_of_month", "click", function(event)
        	   { 
   			   event.stopPropagation();					
       	       var brand = $(this).parent('h5').attr('id');
       	       var brandtitle = $(this).parent('h5').attr('brandtitle');
       	       
       	       $(this).parent('h5').next(".pane-stat").hide();

        	   var plan = prompt("Введите план продаж для "+brandtitle,0);
        	   
        	   if(plan>0) 
        	   		{
        	   		$(this).html("план: "+plan);
        	   		lnk = "do.php?SetPlan="+brand+"&plan="+plan+"&manager="+$.cookie('fpk_id')+"&month="+$('#datediv').html();
        	   		console.info(lnk);
        	   		$.getJSON( lnk ,function(data)
						{
						});
...
//		alert($(this).attr('cantopen'));
	...
	$("img[class=dotype]").unbind('click').click(function(event){ 
    			...
    			client=$(this).parent('div').parent('div').attr("id");
		  		...
		  		console.info(doid, client);

	if( $(this).attr('title') == "OUT" )
		if( ($.cookie("brand") == 3) ) //если Toyota
	  		if( ($.cookie("fpk_job").indexOf("Руководитель")==-1) ) 
		  		if( ($.cookie("fpk_job").indexOf("Старший")==-1) ) 
   		  			if( ($.cookie("fpk_job").indexOf("директор")==-1) ) 
		  				{
		  				alert ("Чтобы поставить OUT, обратитесь к старшему менеджеру или руководителю");
		  				return false;
			  			}
		...
		if( ($.cookie("brand") == 3) ) //если Toyota
	  		if( ($.cookie("fpk_job").indexOf("Руководитель")==-1) ) 
		  		if( ($.cookie("fpk_job").indexOf("Старший")==-1) ) 
   		  			if( ($.cookie("fpk_job").indexOf("директор")==-1) ) 
		  				{
		  				alert ("Чтобы удалить клиента, обратитесь к старшему менеджеру или руководителю");
		  				return false;
			  			}
...
/////////combobox_run
	var listout = 
		[
            ...
            'OUT2 – Покупка автомобиля другого бренда ()',
            'OUT3 – Покупка ЭТОЙ ЖЕ марки у другого дилера ()',
            'OUT4 – Покупка ЭТОЙ ЖЕ марки внутри Холдинга ()',
            'OUT5 – Покупка другой марки внутри Холдинга ()',
            'OUT52 – Покупка другой марки в другом месте ()',
