...
if (isset($HTTP_GET_VARS['reiting'])) 
 {
    ...
    while (@$sql = mysql_fetch_object ($result))
	 ...
	 $hint[$sql->manager].="������ ".$sql->fio." �� ".$sql->cost1."\n\r";
       }

    //Договора
    $sqlnews="SELECT manager,1_clients.cost cost, 1_models.cost modelcost FROM `1_clients` LEFT JOIN `1_models` ON `1_models`.id=`1_clients`.model WHERE `1_clients`.brand = '".$fpk_brand."' AND dg>'$dat' AND 1_clients.out='0000-00-00 00:00:00' AND manager NOT LIKE 'Показан%' AND manager NOT LIKE 'Балчуг%' ORDER by manager";
    ...
    $sqlnews="SELECT manager,1_clients.cost cost, 1_models.cost modelcost FROM `1_clients` LEFT JOIN `1_models` ON `1_models`.id=`1_clients`.model WHERE `1_clients`.brand = '".$fpk_brand."' AND tst>'$dat' AND 1_clients.out='0000-00-00 00:00:00' AND manager NOT LIKE 'Показан%' AND manager NOT LIKE 'Балчуг%' ORDER by manager";
    ...
    while (@$sql = mysql_fetch_object ($result))
       {
        ...
        $dg[$sql->manager]+=$cost1/5;
        $man[$sql->manager]+=$cost1/5;
       }
    ...
    while (@$sql = mysql_fetch_object ($result))
       {
        ...
        $dg[$sql->manager]+=$cost1/8;
        $man[$sql->manager]+=$cost1/8;
       }
   ...
   for($i=0;$i<count($man);$i++)
      ...
      if (($HTTP_GET_VARS['manager']!='Все') and ($HTTP_GET_VARS['reiting']==1) and ($HTTP_GET_VARS['manager']==key($man))) 
         ...
         if ($i==0) echo '<img src="crown.png" title="Вы лучше всех. Так держать." style="position:absolute;top:23px;left:77px;width:80px;">';
         if ($i==1) echo '<img src="crown.png" title="Поднажми чуть-чуть и станешь лучшим!" style="position:absolute;top:53px;left:105px;width:30px;opacity:0.8">';
         if ($i==count($man)-1) echo '<img src="ushanka.png" title="У вас очень низкие продажи. Встряхнитесь." style="position:absolute;top:40px;left:94px;width:55px;opacity:0.9">';
         //echo $i;
         }

      
      if ($HTTP_GET_VARS['reiting']==3) echo '<li><a title="'.$hint[key($man)].'" href="#nogo28" style="width:250px;float:left;">'.key($man).'</a><a style="width:43px;float:left;text-align:right;">'.$procent.'%</a></li>';
      next($man); 
//print_r($hint);
      }
...
if($dif_sec>10)
{
  ...
  $sqlnews="SELECT count(*) cnt FROM `1_brands` WHERE 1_brands.Show=1";
  $result = mysql_query_my($sqlnews); 
  @$sql = mysql_fetch_object ($result);
  $maxcnt = $sql->cnt;
  ...
  for ($k = 0; $k<= $maxcnt; $k++)
     {
      ...
      <div class="roundfooter2" id="tst" title="Тестдрайвы" style="margin-right:20px">'.$tst.'</div>
      ';
...
if (isset($HTTP_GET_VARS['cupAdmin'])) 
 {
  ...
  for ($k = 0; $k<= $maxcnt; $k++)
    ...
    echo '
      ...
      <img src="img/1test-drive.png" width="15px" align="left" style="padding-right:2px;padding-top:2px;opacity:0.7">
      <div class="roundfooter2" id="tst" title="Тестдрайвы" style="margin-right:20px">'.$tst.'  '.$tst2.'</div>
      ';
...
VALUES (
'',  'Клиент (".gmdate("d.m H:i",cheltime(time())).")',  '',  '',  '',  '',  '',  '".gmdate("Y-m-d",cheltime(time()))."',  '',  '1978-00-00',  '".$fpk_brand."',  '".$Manager."', '".$fpk_job."'
);
   ...
   if ($HTTP_GET_VARS['AddClient']==2) $d=AddDo($sql->maxid,'Звонок',1);
   if ($HTTP_GET_VARS['AddClient']==3) $d=AddDo($sql->maxid,'Визит',1);
   if ($HTTP_GET_VARS['AddClient']==6) { $d=AddDo($sql->maxid,'Визит',1); $d=AddDo($sql->maxid,'Тест-драйв',1); }
