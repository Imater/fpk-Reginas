...
echo "<ul><li class_id='-1'>Все модели</li>";
	...
	$w1 = "(SELECT count(*) cnt FROM cars WHERE state = 'ordered' AND `cars`.klasse_id = `klasses`.id ) cnt1";
	$w2 = "(SELECT count(*) cnt FROM cars WHERE state = 'pending' AND `cars`.klasse_id = `klasses`.id ) cnt2";
	$w3 = "(SELECT count(*) cnt FROM cars WHERE state = 'proposed' AND `cars`.klasse_id = `klasses`.id ) cnt3";
  ...
  while (@$sql = mysql_fetch_array($result))
    {
    echo '<li class_id="'.$sql['id'].'">'.$sql['name'].' <span class="li_count">'.$sql['cnt2'].'+'.$sql['cnt1'].'+'.$sql['cnt3'].'</span></li>';
    }
  echo '</ul>';
...
if($id!=-1)  
  ...
  $sqlnews = "SELECT * FROM cars WHERE $where ORDER by state DESC";
  ...
  while (@$sql = mysql_fetch_array($result))
    {
    $state = "Передан";
    if($sql['state'] == "ordered") $state = "В производстве";
    if($sql['state'] == "pending") $state = "В наличии";
    if($sql['state'] == "proposal") $state = "В резерве";
    
    echo '<h6 group_by="'.$state.'"><span style="display:inline-block;width:40px;margin-right:5px;text-align:right;color:lightgray">'.$i.'.</span> '.$sql['order'].'</h6>';
	echo '<div class="m_about">ha!</div>';
	$i++;
    }
...
if (isset($HTTP_GET_VARS['ShowReiting'])) 
 ...
 if ($manager=='Все') $manager="%";
		...
		$news .= cntSum(4, "SELECT *, 'Тестдрайвы: #VD#' gr from 1_clients WHERE brand='$fpk_brand' AND manager LIKE '$manager' AND tst >= '$d1' AND tst <= '$d2'  AND 1_clients.out = '0000-00-00 00:00:00' LIMIT 600");
		...
		$news .= cntSum(4*5, "SELECT *, 'Тестдрайвы OUT: #VD#' gr from 1_clients WHERE brand='$fpk_brand' AND manager LIKE '$manager' AND tst >= '$d1' AND tst <= '$d2'  AND 1_clients.out != '0000-00-00 00:00:00' LIMIT 600");
...
$addsql="";
if( $fpk_brand == 1 ) $addsql=" OR brand = '13'";
if( $fpk_brand == 13 ) $addsql=" OR brand = '1'";

$sqlnews = "SELECT fio, manager FROM 1_clients WHERE fio LIKE '%$q%' AND ( brand = '$fpk_brand' ".$addsql." ) LIMIT 50";
//echo $sqlnews;
...
$sqlnews = "SELECT phone1, fio, manager FROM 1_clients WHERE phone1 LIKE '%$q%' AND (brand = '$fpk_brand' ".$addsql." ) LIMIT 10 UNION
SELECT phone2, fio, manager FROM 1_clients WHERE phone2 LIKE '$q%' AND (brand = '$fpk_brand' ".$addsql." ) LIMIT 10 UNION
SELECT phone3, fio, manager FROM 1_clients WHERE phone3 LIKE '$q%' AND (brand = '$fpk_brand' ".$addsql." ) LIMIT 10 UNION
SELECT phone4, fio, manager FROM 1_clients WHERE phone4 LIKE '$q%' AND (brand = '$fpk_brand' ".$addsql." ) LIMIT 10";
  ...
  if ($field=='model') $sqlnews="SELECT DISTINCT(model) fio FROM `1_clients` WHERE brand = '$fpk_brand'";
	...
	if ($field=='model')	
		...
		@$conv=(integer)($tst['count'][$gr]/($vz['count'][$gr])*100);
  		...
  		if ($tst['count'][$gr]) $answer['rows'][$i]['cell'][4]=$tst['procent'][$gr].'% <a id="showclients" href="#" title="'.$tst['clients'][$gr].'"><font color="'.$gray.'">('.$tst['count'][$gr].')</font></a> <a title="Конверсия тестдрайвов = Тестдрайвы/Визиты" style="color:darkgreen">'.$conv.'%';
...
if (isset($HTTP_GET_VARS['reiting'])) 
 {
    ...
    while (@$sql = mysql_fetch_object ($result))
      ...
      if ($HTTP_GET_VARS['days']<50)  $hint[$sql->manager].="Тестдрайв ".$sql->fio." за ".$cost1."\n";
      }
...
//  $sqlnews="SELECT count(*) cnt FROM `1_brands`";
  $sqlnews="SELECT count(*) cnt FROM `1_brands` WHERE 1_brands.Show=1 AND id IN (SELECT DISTINCT(brand) FROM 1_clients WHERE brand!='' AND vz>DATE_ADD(NOW(), INTERVAL -3 MONTH)) ORDER by brandname DESC, title DESC";
...
//  $sqlnews="SELECT *, id brand FROM `1_brands` ORDER by brandname DESC, title DESC";
  $sqlnews="SELECT *, id brand FROM `1_brands` WHERE 1_brands.Show=1 AND id IN (SELECT DISTINCT(brand) FROM 1_clients WHERE brand!='' AND vz>DATE_ADD(NOW(), INTERVAL -3 MONTH)) ORDER by brandname DESC, title DESC";
  ...
  for ($k = 0; $k<= $maxcnt; $k++)
    ...
    $sqlnews="SELECT * FROM `1_plan` WHERE brand=".$brand." AND month='".$alldatemonth."'";
    $result1 = mysql_query_my($sqlnews); @$sql1 = mysql_fetch_object ($result1);
    $plan_month = $sql1->plan;

    $js[$brand]['amount'][8]=$plan_month;
...
if (isset($HTTP_GET_VARS['SetPlan'])) 
 {
 $brand = $HTTP_GET_VARS['SetPlan'];
 $plan = $HTTP_GET_VARS['plan'];
 $manager = $HTTP_GET_VARS['SetPlan'];
 $month = $HTTP_GET_VARS['month'];
 $month = substr($month,0,7);
 
 echo $brand." - ".$plan." - ".$manager." : ".$month;
 
  $sqlnews="DELETE FROM `1_plan` WHERE brand='$brand' AND month='$month'";
  $result = mysql_query_my($sqlnews); 
//  @$sql = mysql_fetch_object ($result);

   $sqlnews = "INSERT INTO  `h116`.`1_plan` (
   					`id` ,
   					`brand` ,
   					`plan` ,
   					`manager` ,
...
//  $sqlnews="SELECT * FROM `1_brands` WHERE 1_brands.Show=1 ORDER by brandname DESC, title DESC";
  ...
  $sqlnews="SELECT * FROM `1_brands` WHERE 1_brands.Show=1 AND id IN (SELECT DISTINCT(brand) FROM 1_clients WHERE brand!='' AND vz>DATE_ADD(NOW(), INTERVAL -3 MONTH)) ORDER by brandname DESC, title DESC";
  ...
  for ($k = 0; $k<= $maxcnt; $k++)
     {
      ...
      <div class="roundfooter2" id="tst" title="Тестдрайвы" style="margin-right:0px">'.$tst.'</div>
      ';
...
///////////////////////////////////////////////
    ...
    $short='&nbsp;&nbsp;<font color=#FFF>'.($vd+$plan1+$plan2+$plan3).'</font> = <span title="Выданы">'.$vd.'</span> + <span title="C вероятностью выдачи: 5">'.$plan1.'</span> + <span title="С вероятностью выдачи: 4">'.$plan2.'</span> + <span title="С вероятностью выдачи: 3">'.$plan3.'</span>';
    
    $plan_div = '<div class="plan_of_month" id="prognoz" title="Кликните, чтобы изменить план продаж месяца" style="float:right;width:70px;font-size:12px;color:#b1afaf;text-align:left;">план: '.$plan_month.'</div>';
    
    if ($k==$maxcnt) $plan_div="";
    
    $long='Выданы = '.$vd.' / Подтвержденные: '.($vd+$plan1).' / Вероятные: '.($vd+$plan1+$plan2).' / Маловероятные: '.($vd+$plan1+$plan2+$plan3);

	$mytext .= '<div class="roundfooter2" id="prognoz" title="'.$long.'" style="float:right;width:170px;font-size:12px;color:lightgray;text-align:left;">'.$short.'</div><img src="img/1vidacha.png" width="15px" title="Планируемые выдачи в этом месяце" align="left" style="float:right;padding-right:2px;padding-top:1px;opacity:0.3">'.$plan_div;
	  ...
	  $mytext .=  '<li><a id2=tst href=# brand="'.$brand.'" >Тестдрайвы</a></li>';
...
if (isset($HTTP_GET_VARS['cupAdmin'])) 
 {
  ...
  for ($k = 0; $k<= $maxcnt; $k++)
    ...
    echo '
      ...
      <img src="img/1test-drive.png" width="15px" align="left" style="padding-right:2px;padding-top:2px;opacity:0.7">
      <div class="roundfooter2" id="tst" title="Тестдрайвы" style="margin-right:0px">'.$tst.'  '.$tst2.'</div>
      ';
	...
	if ($sql->checked!="0000-00-00 00:00:00")
	  {
		 ...
		 if ( in_array($t, array("Визит", "Договор", "Кредит", "Тест-драйв") ) AND (!$vz)) { echo "<b> <- Визит: ".$sql->date2."</b>"; $vz=$sql->date2; }

		 if ( in_array($t, array("Договор") ) AND (!$dg)) { echo "<b> <- Договор: ".$sql->date2."</b>"; $dg=$sql->date2; }

		 if ( in_array($t, array("Тест-драйв") ) AND (!$tst)) { echo "<b> <- Тест-драйв: ".$sql->date2."</b>"; $tst=$sql->date2; }
...
if (isset($HTTP_GET_VARS['news'])) 
 {
	...
	$alldatemonth=mb_substr( $HTTP_GET_VARS['date'], 0, 7,'utf-8' );
	$startTime=microtime();
    $sqlnews="SELECT count(*) cnt FROM `1_clients` WHERE icon2=5 AND vd='0000-00-00 00:00:00' AND 1_clients.out='0000-00-00 00:00:00' AND brand='".$fpk_brand."'";
    $result = mysql_query_my($sqlnews); @$sql1 = mysql_fetch_object ($result);
    $plan1 = $sql1->cnt;

    $sqlnews="SELECT count(*) cnt FROM `1_clients` WHERE icon2=4 AND vd='0000-00-00 00:00:00' AND 1_clients.out='0000-00-00 00:00:00' AND brand='".$fpk_brand."'";
    $result = mysql_query_my($sqlnews); @$sql1 = mysql_fetch_object ($result);
    $plan2 = $sql1->cnt;
    
    $sqlnews="SELECT count(*) cnt FROM `1_clients` WHERE icon2=3 AND vd='0000-00-00 00:00:00' AND 1_clients.out='0000-00-00 00:00:00' AND brand='".$fpk_brand."'";
    $result = mysql_query_my($sqlnews); @$sql1 = mysql_fetch_object ($result);
    $plan3 = $sql1->cnt;
...
exit;
 ...
 первичные звонки ".cnt("zv",$alldate)."(".cnt("zv",$alldatemonth)."), первичные визиты ".cnt("vz",$alldate)."(".cnt("vz",$alldatemonth)."), тестдрайвы ".cnt("tst",$alldate)."(".cnt("tst",$alldatemonth)."), план выдач ".(cnt("vd",$alldatemonth)+$planvidach)."(".cnt("vd",$alldatemonth)." уже выдано + $planvidach запланированно).";
 exit;
 }

function addType($type,$sql1,$my_base,$global_date)
{
  
    if($sql1[$type]!="0000-00-00 00:00:00") 
    	{
    	if(($type=="out") && ($sql1["dg"]=="0000-00-00 00:00:00")) return $my_base; //если это аут, то нужен договор
    	
    	$brand = $sql1["brand"];
    	$d = substr($sql1[$type],0,7);
    	$my_base[$type][$brand][$d] = $my_base[$type][$brand][$d]+1;

    	$d = substr($sql1[$type],0,10);
    	if($d == $global_date)	$my_base[$type][$brand][$d] = $my_base[$type][$brand][$d]+1;
    	}

return $my_base;
}

function getBase($date)
{
global $HTTP_GET_VARS;

  $global_date = $HTTP_GET_VARS["date"];
  
  $global_month = substr($global_date,0,7);

  $sqlnews = "SELECT zv,vz,tst,dg,vd,`out`,brand FROM 1_clients WHERE zv > '$global_month' OR vz > '$global_month' OR tst > '$global_month' OR dg > '$global_month' OR vd > '$global_month'  OR `out` > '$global_month' AND brand !=''";
  $result = mysql_query_my($sqlnews); 
  $i=0;
  while(@$sql1 = mysql_fetch_array ($result))
  	{
   	$my_base = addType("out",$sql1,$my_base,$global_date);
   	$my_base = addType("zv",$sql1,$my_base,$global_date);
   	$my_base = addType("vz",$sql1,$my_base,$global_date);
   	$my_base = addType("tst",$sql1,$my_base,$global_date);
   	$my_base = addType("dg",$sql1,$my_base,$global_date);
   	$my_base = addType("vd",$sql1,$my_base,$global_date);
    $i++;
    }
...
function cnt3 ($type, $date, $brand)
{
global $base;

if($type=="out2") $type="out";
...
{
if($brand!="%") return cnt3 ($type, $date, $brand);
...
if (!$brand) $brand=$fpk_brand;
    ...
    $sqlnews="SELECT count(*) cnt FROM `1_clients` cl WHERE (brand!=10 AND brand!=11) $mybr AND $type LIKE '$date%' AND $type <= '$dateto' ";
//    echo $sqlnews;
...
if( $admin!="" )
   ...
   if ($HTTP_GET_VARS['AddClient']==6) { $d=AddDo($sql->maxid,'Тест-драйв',$did,$fpk_user,$Manager); $d=AddDo($sql->maxid,'Визит',1,$fpk_user,$Manager); }
   ...
   if ($HTTP_GET_VARS['AddClient']==7) { $d=AddDo($sql->maxid,'Визит',1); $d=AddDo($sql->maxid,'OUT',1); $d=AddDo($sql->maxid,'Тест-драйв',1); }
