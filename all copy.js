...
function jsLeftDo()
{
 ...
 type = $("* .left-bottom-top a[class=current]").attr('type');
 ...
 $.getJSON( lnk ,function(data)
	...
	$.each(data, function(i,data)
		  ...
		  if (j==0)
		    {
		    $('.cnt_today').html(data[0].cnt_today);
		    $('.cnt_did').html(data[0].cnt_did);
		    $('.cnt_past').html(data[0].cnt_past);
		    }
		  j=1;
		  });
...
function jsMenuFirst(menu_parent)
{
  	 ...
  	 $('*').undelegate("* .left-bottom-top a", "click").delegate("* .left-bottom-top a", "click", function(event) 
		{
			event.stopPropagation();										    
		    type=$(this).attr('type');
		    $("* .left-bottom-top a").removeClass('current');
  	...
  	$('*').undelegate(".tabs li", "click").delegate(".tabs li", "click", function(event) 
		{
			...
			if (childs>0) { jsMenu(radio); return false; }
...
function jsSQLClients(radio)
{
 ...
 if ( radio==30 ) { $('.accordion2').show(); jsShowDo(4); return true; }
...
setTimeout( function()
             {
			  $('.title').css("opacity",'0').html('<b>'+title+'</b>').animate({"opacity": '1'}, 1000, function()
								{ 
								setTimeout(function()
   										{
...
$('#reiting3').load(lnk,function()
	...
	lnk = "do.php?reiting=3&date="+alldate+"&days=31&manager="+ manager;
