...
function AddDo($client,$Type,$did,$admin,$Manager)
{
...
VALUES (
'',  '".$client."',  '".$fpk_brand."',  '".$man."',  '".$ddd."',  '".$ddd."',  '".$Type."',  '',  '".$d."',  '".$Type."',  '".$host."',  '50',  '',  '0000-00-00 00:00:00',  '".$ddd."',  '0000-00-00 00:00:00',  '',  '0000-00-00 00:00:00',  '')
";
...
{
if($sql->model==0) return '';

$cost = (integer)($sql->cost/1000);
if ($cost==0) 
  {
   if($sql->mycost) $mycost = $sql->mycost;
   else
     {
   ...
   $cost = '~'.(integer)($mycost)/1000;
   if ($mycost<300000) $cost='~444';
  }
...
{
if($sql->mymodel) return $sql->mymodel;
...
function mod_ShowDoTypeList()
{
   ...
   $sqlnews="SELECT type FROM 1_dotype WHERE type != 'OUT' ORDER by typeorder";
   $result = mysql_query_my($sqlnews); 
   $TXT='';
   while (@$sql = mysql_fetch_object ($result))
     {
...
function User2 ($email, $pass)
{
global $fpk_user,$fpk_user_short,$fpk_brand,$fpk_id,$fpk_job;
   $sqlnews1="SELECT fio,job,id FROM 1_users WHERE email='".$email."' AND md5password='$pass'";
   $result1 = mysql_query_my($sqlnews1); 
   @$sql1 = mysql_fetch_object ($result1);
...
//   $sqlnews="INSERT INTO `1_log` (`id`, `date1`, `manager`, `brand`, `text`, `client`, `important`,`sqlnews`) 
  //           VALUES ('', NOW(), '$fpk_user', '$fpk_brand', '$howLong', '', '$important','".$sqlnews."');";
//
//   if ($howLong >= -1) @$result2 = mysql_query($sqlnews); 
